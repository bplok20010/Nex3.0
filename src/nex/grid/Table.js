...
(function( factory ) {
	...
	var dataGrid = Nex.define('Nex.grid.Table',{
		...
		configs : function(opt){
			return {
				...
				tpl : {
					...
					'view2_header_inner_row_bak' : '<tr class="nex-grid-view-header-row">'
													+'<% var i=0;len = fields.length;  for(;i<len;i++) {%>'
													+'<td id="<%=opt.id%>_cols_<%=fields[i]["_colid"]%>" class="nex-grid-td datagrid_<%=fields[i]["_colid"]%>" field="<%=fields[i]["field"]%>" align="<%=fields[i]["align"]%>">'
														+'<div class="nex-column-header-inner" field="<%=fields[i]["field"]%>" >'
	...
	dataGrid.override({
		...
		onBodyCreate : function(bd,opt){
			...
			self.setGridViewLayout(bd,opt);
			
			self.setGridViewHeader();
			self.setGridViewBody();
		},
		getGridViewBody : function(){
			return this.configs.views['dataViewBody'];	
		},
		gridViewRendered : false,
		setGridViewLayout : function(bd,opt){
			if( this.gridViewRendered ) {
				return this;	
			}
			this.gridViewRendered = true;
			var $header = $('<div class="nex-grid-view-header" id="'+opt.id+'_dataview_header" style="width: 100%;">'
						+ '    <div class="nex-grid-view-header-inner" id="'+opt.id+'_dataview_header_inner">'
						+ '        <div class="nex-grid-view-header-inner-wraper" id="'+opt.id+'_dataview_header_inner_wraper">'
						+ '        </div>'
						+ '    </div>'
						+ '</div>');
			var $body = $('<div class="nex-grid-view-body" id="'+opt.id+'_dataview_body"></div>');			
			var $view = $('<div class="nex-grid-dataview" id="'+opt.id+'_dataview"></div>');				
			...
			opt.views['dataViewHeader'] = $header;
			opt.views['dataViewBody'] = $body;
			
			return this;
		},
		//重载
		$$initViewContent : function(){
			var self = this;
		...
		setGridViewHeader : function(){
			...
			var headerView = ['<table class="nex-grid-table nex-grid-view-header-table" id="'+opt.id+'_dataview_header_inner_wraper_table" border="0" cellspacing="0" cellpadding="0"><tbody id="'+opt.id+'_dataview_header_inner_wraper_table_tbody">'];
			...
			headerView.push('</tbody></table>');
			
			$('#'+opt.id+'_dataview_header_inner_wraper').html( headerView.join("") );
		...
		setGridViewBody : function(){
			var self = this;	
			var opt = self.configs;
			var bd = self.getGridViewBody();
			var table = self.getGridTable( opt.data );
			bd.html( table );	
		},
		getGridRowTpl : function( data ){
			return this.tpl('view2_row',data || {});
		},
		getGridRows : function(data){
			var self = this;	
			var opt = self.configs;
			var data = data || [];
			var rows = [];
			...
			$.each( data,function(i,d){
				...
				rows.push( self.getGridRowTpl(data) );	
			} );	
			return rows.join('');
		},
		getGridTable : function( data ){
			var self = this;	
			var opt = this.configs;
			var tpl = [
				'<table class="nex-grid-table nex-grid-view-body-table" id="'+opt.id+'_dataview_body_table" cellspacing="0" cellpadding="0" border="0">',
					'<tbody>',
						self.getGridRows( data ),
					'</tbody>',
				'</table>'
			];	
			return tpl.join('');
		...
		*/
		...
		addData : function(data){
			...
			if( async ) {
				...
				for(var n=0;n<len;n++) {
					var data = datas[n];
					data[pk] = self._undef( data[pk] , Nex.uuid() );
					_d.push(data);
				}
