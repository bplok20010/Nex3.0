...
;(function($){
	...
	function _ajaxTips(options){
		...
		$.ajax({
			...
			success : function(data,status){
				...
				//if(options.content) ($("#"+options.tid+"_content").html() == options.content)
	...
	function _topIcon(options) {
		...
		var icon = '<div style="position: absolute; bottom:'+bottom+'px; left:'+left+'px; margin:0px; padding:0px;">'
	...
	*/
	...
	/*显示已经创建好的tips*/
	function _display(options,type){
		var type = typeof type == 'undefined' ? false : true;
		if(type) {
			if( $("#"+options.tid).size() ) {
	...
	/*
		刷新tips内容并显示
	*/
	function _refreshTips(options) {
		if( $("#"+options.tid+"_content").size() ) {
			$("#"+options.tid+"_content").html(options.content);
			_positionTips(options);
		} else {
			$.jTips.show(options);			
		}
	...
	*/
	$.jTips.create = function(options){
		...
		if( od.size() ) {
			if( od.is(":hidden") && ($("#"+options.tid+"_content").html() != options.content) ) {//显示中的不需要重新生成,而且如果内容有变动的话 下次显示时 会重新生成
				od.html(container);	
			}
		} else {
	...
	$.fn.jTips = function(opt){
		...
		$(this).each(function(i){
			...
			if( $(this).attr( "data-options" ) ) {
				var opt2 = "{"+ $(this).attr( "data-options" ) +"}";
				opt2 = eval("("+opt2+")");
			} else {
				var opt2 = {};	
			}
			
			var options = $.extend(true,{},$.jTips.defaults,opt2,opt);
			if(options.randomid) {
